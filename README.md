...
2. `probe-access`: classify whether each document exposes a direct PDF URL.
3. `fetch-pdfs`: download only documents that probed as directly retrievable.
4. `extract-text`: run native PDF extraction first and OCR only when needed.
5. `package-corpus`: merge metadata, probe, fetch, and extraction outputs into a final JSONL corpus.
6. `summarize-manifest`: estimate storage needs from manifest file sizes.
7. `summarize-probe`: summarize which documents are directly downloadable.
